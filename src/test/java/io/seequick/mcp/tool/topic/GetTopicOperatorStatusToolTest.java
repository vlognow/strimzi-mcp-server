...
class GetTopicOperatorStatusToolTest {

    KubernetesClient client;

    private GetTopicOperatorStatusTool tool;

    @BeforeEach
    void setUp() {
        tool = new GetTopicOperatorStatusTool(KubernetesClientResolver.fixed(client));
    }

    @Test
    void getNameShouldReturnCorrectName() {
        var spec = tool.getSpecification();
        assertThat(spec.tool().name()).isEqualTo("get_topic_operator_status");
    }
    ...
    void executeShouldReturnNotFoundWhenNoPod() {
        ...
        McpSchema.CallToolRequest request = new McpSchema.CallToolRequest("get_topic_operator_status", args);
        ...
        assertThat(content).contains("Topic Operator Status");
